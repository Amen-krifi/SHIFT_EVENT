...
async function checkAuth() {
  if (!supabaseClient) {
    ...
    if (loginError) {
      loginError.textContent = "Supabase configuration missing or client not initialized.";
...
function renderApplicantsTable() {
  ...
  tbody.innerHTML = filtered
    ...
    <tr class="border-b border-surface-border/50 hover:bg-surface-card-hover/40 transition-colors">
      ...
      <td class="p-3 text-center">
        <button type="button" class="applicant-delete-btn p-1.5 rounded-lg text-text-muted hover:text-error hover:bg-surface-container transition-colors" data-id="${a.id}" data-name="${escapeHtml(a.full_name)}" title="Remove applicant">
  ...
  // Attach click listener to applicant delete buttons
  tbody.querySelectorAll(".applicant-delete-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      applicantsCache = applicantsCache.filter((a) => a.id !== id);
      ...
      if (supabaseClient && isUuid(id)) {
        try {
          ...
          if (error) {
            console.error("Error removing applicant from Supabase:", error);
            alert("Could not remove from database: " + error.message);
            loadApplicants(); // rollback
          }
        } catch (err) {
          console.warn("Error removing applicant:", err);
        }
      }
    });
  });
...
document.getElementById("send-reminders-btn").addEventListener("click", async () => {
  ...
  try {
    if (!supabaseClient || !supabaseClient.functions) {
      statusEl.textContent = "Supabase client not connected.";
      return;
    }
    const { data, error } = await supabaseClient.functions.invoke("send-reminders", { body: {} });
    if (error) {
      statusEl.textContent = "Email service not configured: " + (error.message || "Edge function not found");
      return;
    }
    statusEl.textContent = `Done — sent ${data?.sent || 0}/${data?.total || 0} reminder emails${data?.failed ? `, ${data.failed} failed` : ""}.`;
    loadApplicants();
  } catch (err) {
    statusEl.textContent = "Email function not active. Direct database storage is working.";
  }
});
...
async function loadPartnerRequests() {
  ...
  if (partnerRequestsCache.length === 0) {
    if (tbody) tbody.innerHTML = `<tr><td class="p-3 text-text-muted" colspan="8">No partner applications yet.</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = partnerRequestsCache
    ...
    <tr class="border-b border-surface-border/50 align-top hover:bg-surface-card-hover/40 transition-colors">
      ...
      <td class="p-3 text-center">
        <button type="button" class="partner-delete-btn p-1.5 rounded-lg text-text-muted hover:text-error hover:bg-surface-container transition-colors" data-id="${p.id}" data-name="${escapeHtml(p.company_name)}" title="Remove partner application">
          <span class="material-symbols-outlined text-[18px]">delete</span>
        </button>
      </td>
    </tr>`
  ...
  // Attach delete handlers for partner applications
  tbody.querySelectorAll(".partner-delete-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const id = btn.dataset.id;
      const name = btn.dataset.name || "this partner application";
      if (!confirm(`Are you sure you want to remove the partner application from ${name}?`)) {
        return;
      }

      partnerRequestsCache = partnerRequestsCache.filter((p) => p.id !== id);
      document.getElementById("partner-requests-count").textContent = `(${partnerRequestsCache.length})`;
      updateMetricsDashboard();
      loadPartnerRequests();
      ...
      if (supabaseClient && isUuid(id)) {
        try {
          ...
          if (error) {
            console.error("Error removing partner application:", error);
            alert("Could not remove partner application: " + error.message);
            loadPartnerRequests();
          }
        } catch (err) {
          console.warn("Error removing partner application:", err);
        }
      }
    });
  });
